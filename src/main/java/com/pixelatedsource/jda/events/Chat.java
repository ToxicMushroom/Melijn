...
public class Chat extends ListenerAdapter {
    ...
    public void onGuildMessageReceived(GuildMessageReceivedEvent event) {
        if (event != null && event.getMember() != null) {
            ...
            new Thread(() -> mySQL.createMessage(event.getMessageId(), event.getMessage().getContentRaw(), author.getId(), guild.getId(), event.getChannel().getId())).start();
    ...
    public void onGuildMessageDelete(GuildMessageDeleteEvent event) {
        ...
        Guild guild = event.getGuild();
        if (SetLogChannelCommand.logChannelIds.containsKey(guild.getId()) && event.getGuild().getSelfMember().hasPermission(Permission.VIEW_AUDIT_LOGS)) {
            executorService.execute(() -> {
                JSONObject message = mySQL.getMessageObject(guild.getId(), event.getMessageId());
                User user = event.getJDA().retrieveUserById(message.getString("authorId")).complete();
                if (user != null && !black.contains(user)) {
                    ...
                    if (MessageHelper.filterDeletedMessages.get(event.getMessageId()) != null) {
                        ...
                        User deletor = sameAsLast ? auditLogEntry.getUser() : event.getJDA().getUserById(PixelSniper.mySQL.getMessageAuthorId(guild.getId(), event.getMessageId()));
                        if (deletor != null) eb.setFooter("Deleted by: " + deletor.getName() + "#" + deletor.getDiscriminator(), deletor.getAvatarUrl());
                    }

                    guild.getTextChannelById(SetLogChannelCommand.logChannelIds.get(guild.getId())).sendMessage(eb.build()).queue(v -> {
