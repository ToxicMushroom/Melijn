    ...
    protected void execute(CommandEvent event) {
        if (event.getGuild() != null) {
            if (Helpers.hasPerm(event.getMember(), commandName, 1)) {
                ...
                String role = PixelSniper.mySQL.getRoleId(guild, RoleType.MUTE);
                if (args.length == 0 || args[0].equalsIgnoreCase("")) {
                    if (role != null && role.matches("\\d+") && guild.getRoleById(role) != null)
                        ...
                        event.reply("Current MuteRole: **null**");
                } else {
                    ...
                    if (PixelSniper.mySQL.setRole(guild, muteRoleId, RoleType.MUTE)) {
