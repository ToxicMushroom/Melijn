        ...
        if (event.getGuild() == null || event.hasPerm(event.getMember(), commandName, 0)) {
            ...
            if (args.length == 0 || args[0].isEmpty()) {
                event.getWebUtils().getImage("triggered",
                        image -> event.getMessageHelper().sendFunText("**" + event.getAuthor().getName() + "** is triggered", image.getUrl(), event)
                );
                ...
                if (target == null && role == null) {
                    ...
                    event.getWebUtils().getImage("triggered",
                            image -> event.getMessageHelper().sendFunText("**" + event.getAuthor().getName() + "** got triggered by **" + role.getAsMention() + "**", image.getUrl(), event)
                    );
                }
            } else {
