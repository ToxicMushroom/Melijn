...
class UserInfoCommand : AbstractCommand("command.userinfo") {
    ...
    suspend fun execute(
        ...
        @CommandArg(0, optional = true) user: User?
    ) {
        val target: User = user ?: context.author

        val member: Member? = if (context.isFromGuild) context.guild.retrieveMember(target).awaitOrNull() else null
        val isSupporter = context.daoManager.supporterWrapper.getUsers().contains(target.idLong)
        ...
        val value1 = replaceUserVar(unReplacedValue1, target, isSupporter, yes, no)
