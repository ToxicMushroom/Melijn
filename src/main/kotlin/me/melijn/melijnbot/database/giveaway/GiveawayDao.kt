...
class GiveawayDao(driverManager: DriverManager) : Dao(driverManager) {

    override val table: String = "giveaways"
    override val tableStructure: String = "guildId bigint, channelId bigint, messageId bigint, winners int, prize varchar(128), endTime bigint"
    ...
    suspend fun insertOrUpdate(guildId: Long, giveaway: Giveaway) {
        giveaway.apply {
    ...
    suspend fun remove(guildId: Long, messageId: Long) {
        driverManager.executeUpdate("DELETE FROM $table WHERE guildId = ? AND messageId = ?",
            guildId, messageId)
    }
}
